...
from settings import W, H, CELL_SIZE, DOMINO_BACKGROUND_COLOR, DOMINO_BORDER_COLOR, DOMINO_DOT_COLOR, \
     TRANSPARENT_COLOR, STORAGE_PANE_COORDS, POOL_DOMINO_INTERVAL, \
...
class Domino:
    ...
    def create_coords(self):
        ...
        left_x0, left_y0 = - (dx * 3), 0 # left_x0 = -30, left_y0 = 0
        right_x0, right_y0 = dx * 3, 0 # right_x0 = 30, right_y0 = 0
    ...
    def create_surface(self):
        # Формируем опорные величины
        x1 = self.corner_points[0][0]
        y1 = self.corner_points[0][1]
        x2 = self.corner_points[1][0]
        y2 = self.corner_points[1][1]
        ...
        pg.draw.rect(surface, DOMINO_BORDER_COLOR, (0, 0, width, height), 3)
...
class Chain:
    ...
    def center_line(self):
        # chain_elements - список домино, которые сыграны
        if not self.chain_elements:
            return 0
...
class Storage:
    ...
    STORAGE_HEIGHT = CELL_SIZE * 3
    CIRCLE_COLOR = (50, 150, 0)
    FONT_COLOR = (255, 255, 255)

    def __init__(self, player_pool, chain):
        self.player_pool, self.chain = player_pool, chain
        self.domino_list = [Domino(side1, side2) for side1 in range(7) for side2 in range(side1, 7)]
        random.shuffle(self.domino_list)
        self.surface = pg.Surface((self.STORAGE_WIDTH, self.STORAGE_HEIGHT))
        self.surface.set_colorkey(TRANSPARENT_COLOR)
